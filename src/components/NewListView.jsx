...
export default class NewListView extends Component {
    ...
    addNewItem = (e) => {
        if(this.habitInput.current.value !== "") {
            ...
            [...this.state.currentHabitList, {text: this.habitInput.current.value, selectedTime:this.state.selectedTime.toLocaleTimeString()} ]})
    ...
    notifier = () => {
        const notification = new Notification("yo");
    }
    ...
    render() {
        ...
        const habit = this.state.currentHabitList.map((habit, index) => {
            return <Habit key={index} listId={index} 
            habitName={habit.text} deleteItem={this.deleteItem} 
            time={this.props.time} habitList={this.state.currentHabitList}
            />
        })
        const time = new Date();
        setTimeout(() => {
        ...
        const saveButtonRenderer = () => {
            if(this.state.currentHabitList.length >= 1) {
                saveButton = <button className="button" onClick={this.resetList}>Save list</button>
            } 
            else {
                saveButton = "";
            }
        ...
        return (
            <div className="NewListViewContainer">
                <div id="habitAdder">                
                    ...
                    <MuiPickersUtilsProvider className="timeSelector"utils={DateFnsUtils}>
                    ...
                    <button className="button"onClick={this.addNewItem}>Add Habit</button>
                </div>
